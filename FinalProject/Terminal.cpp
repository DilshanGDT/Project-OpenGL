...
void loadTerminalTextures() {
	...
	const char* textureFiles[] = {
		"C:/Users/Dilshan/Documents/VS Code/CSC_3081/Sample Program/images/terminal/glass.jpg", 
		"C:/Users/Dilshan/Documents/VS Code/CSC_3081/Sample Program/images/terminal/door.jpg"
	};

	for (int i = 0; i < 2; i++) {
		terminalTextures[i] = SOIL_load_OGL_texture(
			textureFiles[i],
			SOIL_LOAD_AUTO,
			SOIL_CREATE_NEW_ID,
...
void singleWallSide_terminal() {
	...
	glBindTexture(GL_TEXTURE_2D, terminalTextures[0]);

	glPushMatrix();
	glBegin(GL_QUADS);
	glColor3f(1.0f, 1.0f, 1.0f);
	glTexCoord2f(0.0f, 0.0f); glVertex3f(-10, 0, 0);
	glTexCoord2f(1.0f, 0.0f); glVertex3f(10, 0, 0);
	glTexCoord2f(1.0f, 1.0f); glVertex3f(10, 4, 0);
	glTexCoord2f(0.0f, 1.0f); glVertex3f(-10, 4, 0);
	glEnd();
	glPopMatrix();
...
void doorPanel_terminal() {
	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, terminalTextures[1]);
